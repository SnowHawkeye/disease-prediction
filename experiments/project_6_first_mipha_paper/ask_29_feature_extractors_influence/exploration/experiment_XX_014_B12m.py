...
Data: t2d_B24m_G3m_P1y, with selection of the last 12 timesteps (similar to t2d_B12m_G3m_P1y)
Model summary: 3D data, Transformers, RandomOverSampler
Observations: Results are very close to the same experiment with 24 timesteps.
"""
...
def setup_components(data_sources_train, data_sources_test, labels_train, labels_test):
    ...
    # keeping the last 12 timesteps
    for data_source in data_sources_train: data_source.data = data_source.data[:, -12:, :]
    for data_source in data_sources_test: data_source.data = data_source.data[:, -12:, :]
